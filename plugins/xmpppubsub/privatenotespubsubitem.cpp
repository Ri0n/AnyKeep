...
namespace {
    ...
    bool isPayloadChild(const QDomElement &element, const QString &name)
    {
        if (element.isNull() || elementLocalName(element) != name)
            return false;
        if (element.namespaceURI() == PrivateNotesPubSubItem::payloadNamespace)
            return true;
        ...
        if (element.hasAttribute(QStringLiteral("xmlns")))
            return element.attribute(QStringLiteral("xmlns")) == PrivateNotesPubSubItem::payloadNamespace;
        return true;
    }
    ...
    bool hasUnknownCoreChildren(const QDomElement &element)
    {
        for (auto node = element.firstChild(); !node.isNull(); node = node.nextSibling()) {
            ...
            if (child.namespaceURI().isEmpty() || child.namespaceURI() == PrivateNotesPubSubItem::payloadNamespace)
                return true;
        }
        return false;
    }
...
void PrivateNotesPubSubItem::parsePayload(const QDomElement &element)
{
    ...
    if (element.namespaceURI() != payloadNamespace || element.hasAttribute(QStringLiteral("wire"))
        ...
        parseError_   = QStringLiteral("Obsolete pre-unified private-note encrypted payload");
        return;
    }
    if (hasUnknownCoreAttributes(element) || hasUnknownCoreChildren(element)) {
        parseError_ = QStringLiteral("Unknown core field in encrypted private-note payload");
        return;
    }
    ...
    if (payload_.id.isEmpty() || encodedKeyId.size() != 43 || !isBase64Alphabet(encodedKeyId, true, false)) {
        parseError_ = QStringLiteral("Incomplete or malformed encrypted private-note payload");
        return;
    }
    ...
    if (payload_.keyId.size() != 32
        ...
        parseError_ = QStringLiteral("Non-canonical key ID in encrypted private-note payload");
        return;
    }
    ...
    if (nonceElements.size() != 1 || payloadElements.size() != 1 || tagElements.size() != 1
        ...
        parseError_ = QStringLiteral("Incomplete or malformed encrypted private-note XML envelope");
        return;
    }
    ...
    if (compactBase64(payloadElements.constFirst().text()).size() > MaximumEncodedPayloadSize) {
        ...
        parseError_   = QStringLiteral("Encrypted private-note payload exceeds the implementation size limit");
        return;
    }
    ...
    if (!simpleBinaryElement(nonceElements.constFirst()) || !simpleBinaryElement(payloadElements.constFirst())
        ...
        parseError_ = QStringLiteral("Invalid Base64 in encrypted private-note XML envelope");
        return;
    }
    if (payload_.nonce.size() != 12 || payload_.tag.size() != 16 || payload_.cipherText.isEmpty()) {
        parseError_ = QStringLiteral("Invalid encrypted private-note XML envelope fields");
        return;
    }
