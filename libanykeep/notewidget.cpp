...
NoteWidget::NoteWidget(const Note &note, const QUuid &draftId) : ui(new Ui::NoteWidget)
{
    ...
    speechAction   = AnyKeep::initAction(speechIcon, tr("Mic"), tr("Hold to dictate text"), "", this);
...
{
    return AnyKeep::initAction(icon ? QIcon(icon) : QIcon(), title, toolTip, hotkey, this);
}
