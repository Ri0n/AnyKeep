...
namespace {
    ...
    QString markdownFromPlainText(const QString &text)
    {
        ...
        for (const QString &line : normalized.split(QLatin1Char('\n'), Qt::KeepEmptyParts)) {
            ...
            for (const QChar character : line) {
                ...
                // CommonMark permits a backslash before every ASCII punctuation
                // character. Escaping the complete set keeps plain note text
                // literal even with Qt versions whose Markdown writer emits
                // unescaped block markers and inline formatting characters.
                if (value >= 0x21 && value <= 0x7e && !character.isLetterOrNumber())
                    escaped += QLatin1Char('\\');
                escaped += character;
            }
            escapedLines.append(escaped);
        }
        QString markdown = escapedLines.join(QStringLiteral("\n\n"));
    ...
    QString markdownWithGithubUnderlines(const QTextDocument *source)
    {
        ...
        for (int index = 0; index < ranges.size(); ++index) {
            markers.append({ QStringLiteral("ANYKEEPINSOPEN7F3A%1END7F3A").arg(index),
                             QStringLiteral("ANYKEEPINSCLOSE7F3A%1END7F3A").arg(index) });
        }
    ...
    QString protectGithubUnderlineTags(QString markdown)
    {
        ...
        const QString                   openMarker  = QStringLiteral("ANYKEEPINSOPEN7F3A");
        const QString                   closeMarker = QStringLiteral("ANYKEEPINSCLOSE7F3A");
        QString                         result;
...
QString NoteTransferController::plainTextForFragment(const NoteFragment &fragment, QString *error)
{
    ...
    if (error && !error->isEmpty())
        return {};
    static const QString hardBreakMarker   = QStringLiteral("ANYKEEPPLAINHARDLINEBREAK8D27");
                                                  ...
                                                  Note::Format targetFormat)
{
    if (sourceFormat == targetFormat)
        return text;
    if (sourceFormat == Note::PlainText && targetFormat == Note::Markdown)
        return markdownFromPlainText(text);
