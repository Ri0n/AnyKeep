...
public:
    ...
    bool start()
    {
        ...
        temporaryDir = std::make_unique<QTemporaryDir>(QDir::tempPath() + QStringLiteral("/anykeep-audio-XXXXXX"));
    ...
    void finalize()
    {
        ...
        if (!portableContainer || actual.audioCodec() != QMediaFormat::AudioCodec::AAC) {
            fail(AudioAttachmentRecorder::tr(
                "The audio backend substituted a format that is not portable between AnyKeep platforms."));
            return;
        }
