...
namespace AnyKeep {
namespace {
    int documentEndImpl(const QTextDocument *document)
    {
    ...
    QString markdownRangeImpl(QTextDocument *document, int start, int end)
    {
        if (!document)
            return {};
        const int limit = documentEndImpl(document);
    ...
    const QString LinkLabelMarkerPrefix = QStringLiteral("ANYKEEPLINKLABEL7F3A");
    ...
    QString markdownWithSerializedInlineFormats(QTextDocument *document)
    {
        ...
        for (int index = 0; index < links.size(); ++index) {
            linkMarkers.append(QStringLiteral("ANYKEEPSERIALIZEDLINK7F3A%1END7F3A").arg(index));
        }
        QVector<QString> codeMarkers;
        codeMarkers.reserve(codes.size());
        for (int index = 0; index < codes.size(); ++index)
            codeMarkers.append(QStringLiteral("ANYKEEPSERIALIZEDCODE8C53%1END8C53").arg(index));
        ...
        for (int index = 0; index < underlines.size(); ++index) {
            underlineMarkers.append({ QStringLiteral("ANYKEEPINSOPEN8C53%1END8C53").arg(index),
                                      QStringLiteral("ANYKEEPINSCLOSE8C53%1END8C53").arg(index) });
        }
    ...
    QList<NoteBlockSelectionRange> decodeSelectionRangesImpl(const QVariantList &encodedRanges)
    {
...
namespace EditorOperationsPrivate {

    int documentEnd(const QTextDocument *document) { return documentEndImpl(document); }

    QString markdownRange(QTextDocument *document, int start, int end)
    {
        return markdownRangeImpl(document, start, end);
    }

    QList<NoteBlockSelectionRange> decodeSelectionRanges(const QVariantList &encodedRanges)
    {
        return decodeSelectionRangesImpl(encodedRanges);
    }
...
void NoteEditor::applyInlineHtmlFormatting(QQuickTextDocument *quickDocument) const
{
    ...
    static const QString openMarker  = QStringLiteral("ANYKEEPINSOPEN7F3A");
    static const QString closeMarker = QStringLiteral("ANYKEEPINSCLOSE7F3A");
    auto                *document    = quickDocument->textDocument();
