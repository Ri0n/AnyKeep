...
const QString &Utils::anykeepDataDir()
{
    ...
    if (dataDir.isEmpty()) {
        ...
        if (!created)
            qCWarning(logDataPaths) << "Could not create AnyKeep data directory" << dataDir;
#else
        QSettings s;
...
bool Utils::setAutostartEnabled(bool enabled)
{
    ...
    const QString path = home.absoluteFilePath(QStringLiteral("Library/LaunchAgents/com.github.ri0n.AnyKeep.plist"));
    if (!enabled)
    ...
    xml.writeTextElement(QStringLiteral("string"), QStringLiteral("com.github.ri0n.AnyKeep"));
