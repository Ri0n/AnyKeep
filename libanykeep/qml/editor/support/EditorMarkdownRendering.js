...
function formattedLinkLabelRuns(label)
{
    ...
    function appendRun()
    {
        if (buffer.length === 0)
            return runs.push(
                { text : buffer, bold : bold, italic : italic, strike : strike, code : codeDelimiter.length > 0 })
            buffer = ""
    }
    ...
    {
        let end = position
        while (end < label.length && label.charAt(end) === "`")++ end
    ...
    function isWordCharacter(character)
    {
        if (!character)
            return false
            return /[0-9A-Za-z]/.test(character) || character.toUpperCase() !== character.toLowerCase()
    }

    function underscoreIsIntraword(position, length)
    {
        const previous = position > 0 ? label.charAt(position - 1) : ""
        const next = position + length < label.length ? label.charAt(position + length) : ""
    ...
    for (let index = 0; index < label.length;) {
        ...
        if (codeDelimiter.length === 0) {
            ...
            if (character === "*" || (character === "_" && !underscoreIsIntraword(index, 1))) {
                appendRun()
                italic          = !italic
                foundFormatting = true ++index
                continue
            }
        }
...
function markdownForRendering(source)
{
    ...
    if (!rendered
        || (rendered.indexOf("*") < 0 && rendered.indexOf("_") < 0 && rendered.indexOf("~") < 0
            && rendered.indexOf("`") < 0))
    return rendered
        ...
        if (offset > 0 && wholeText.charAt(offset - 1) === "!")
            ...
            for (const run of runs)
            {
                if (run.text.length === 0)
                    continue result += "[" + renderFormattedLinkRun(run) + "](" + destination + ")"
            }
