...
public:
    PluginsIterator()
    {
        ...
        pluginsDirs << Utils::anykeepDataDir() + "/plugins";
...
PluginManager::PluginManager(Main *parent) :
    PluginListSource(parent), anykeep(parent), pluginHost(new PluginHost(this)),
    safeMode_(qApp && qApp->property("anykeepSafeMode").toBool())
{
    updateMetadata();
    if (safeMode_) {
    ...
    connect(pluginHost, &PluginHost::spellCheckProviderConflict, this,
            [this](const QString &activeName, const QString &ignoredName) {
                anykeep->notify(
                    ...
                    tr("AnyKeep is using %1. %2 is also enabled, but is not being used.").arg(activeName, ignoredName),
                    tr("Configure plugins"),
                    [this]() { QMetaObject::invokeMethod(anykeep, "showOptions", Qt::QueuedConnection); });
            });
}
...
void PluginManager::loadPlugins()
{
    ...
    if (nativeExternalTrayAvailable)
        anykeep->setExternalTrayAvailable(true);
    ...
    foreach (const QString &plugin, trayPlugins) {
        ...
        TrayImpl      *tray = tp->initTray(anykeep);
        if (tray) {
            anykeep->setTrayImpl(tray);
    ...
    foreach (const QString &plugin, dePlugins) {
        ...
        anykeep->setDesktopImpl(desktop);
    ...
    foreach (const QString &plugin, stickyPlugins) {
        ...
        if (auto *host = qobject_cast<StickyNotesHostInterface *>(pd->instance)) {
            host->initializeStickyNotes(anykeep->stickyNotesManager());
            anykeep->stickyNotesManager()->setRequiresApplicationAutostart(
        ...
        if (!sticky || !sticky->isStickyNotesAvailable())
            continue;
        anykeep->setStickyNotesImpl(sticky);
        pd->loadStatus = LS_Initialized;
    ...
    foreach (const QString &plugin, gsPlugins) {
        ...
        anykeep->setGlobalShortcutsImpl(qobject_cast<GlobalShortcutsInterface *>(pd->instance));
        pd->loadStatus = LS_Initialized;
    ...
    foreach (const QString &plugin, nPlugins) {
        ...
        anykeep->setNotificationImpl(qobject_cast<NotificationInterface *>(pd->instance));
        anykeep->setActionNotificationImpl(qobject_cast<ActionNotificationInterface *>(pd->instance));
        pd->loadStatus = LS_Initialized;
...
void PluginManager::updateMetadata()
{
    ...
    while (!it.isFinished()) {
...
#endif
        if (loaderMetadata.value(QStringLiteral("IID")).toString() != QLatin1String(ANYKEEP_PLUGIN_INTERFACE_IID)) {
            it.next();
            continue;
        }
        ...
        if (!pluginMetadataFromJson(loaderMetadata, locale, &metadata, &metadataError)) {
            qDebug("Invalid AnyKeep plugin metadata in %s: %s", qPrintable(fileName), qPrintable(metadataError));
            it.next();
            continue;
        }
#ifdef ANYKEEP_DEVEL
        ...
        if (!semanticVersionInRange(QString::fromLatin1(ANYKEEP_VERSION_STR), metadata.minVersion, metadata.maxVersion,
                                    &compatibilityError)) {
            const auto reason = compatibilityError.isEmpty() ? QStringLiteral("version is outside the supported range")
                                                             : compatibilityError;
            qDebug().noquote() << "Incompatible AnyKeep plugin" << fileName << "application version"
                               << ANYKEEP_VERSION_STR << "supported range" << metadata.minVersion << ".."
        ...
        if (!pluginFeaturesFromNames(pd->metadata.features, &pd->features, &unknownFeature)) {
            qDebug("Unknown feature %s in AnyKeep plugin metadata %s. ignore it", qPrintable(unknownFeature),
                   qPrintable(fileName));
            it.next();
            continue;
        }
...
PluginManager::LoadStatus PluginManager::loadPlugin(const QString &fileName, PluginData::Ptr &cache)
{
    ...
    if (!qnp) {
        loader.unload();
        qDebug("not a compatible AnyKeep plugin %s. ignore it", qPrintable(fileName));
