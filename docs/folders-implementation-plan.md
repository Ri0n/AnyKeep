...
- There is one global AnyKeep folder tree, shared by all storages. A storage
...
AnyKeep must not write folder data into Tomboy XML, Tomboy tags, or an invented
Tomboy notebook convention. Existing AnyKeep tags continue to map to Tomboy
...
| --- | --- | --- |
| Folder | A node in the single AnyKeep folder tree | global FolderCatalog, plus providers that can synchronize it |
...
Folder UUIDs are generated by AnyKeep. Folder names are unique among siblings;
...
Failure to read it must never prevent AnyKeep from starting: the user can
...
parents are distinct. If external changes make a path ambiguous, AnyKeep
...
| PTF | .anykeep-folders.json beside the PTF notes | atomic catalog/index; preserve current note IDs and media layout rather than moving files |
...
gets a hidden atomic index file named .anykeep-folders.json:
...
using the dedicated `urn:xmpp:private-notes:folders:0` extension namespace. It does
not publish a provider-global UUID catalog: on import the path is reconciled
into AnyKeep's shared catalog through a persistent provider-path hint, and on
...
folder path. AnyKeep maps a global folder's visible ancestor path to that
...
not be confused with the AnyKeep folder favorite attribute.
...
actions, and writes only the encrypted AnyKeep overlay for notes that do not
...
| reorder toolkit | libanykeep/qml/reorder tree primitives and task-list migration |
