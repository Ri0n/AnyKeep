...
private slots:
    ...
    void horizontalWholeListDragDoesNotDisplaceFollowingBlocks()
    {
        ...
        const auto blockDelegate = [](QQuickItem *item) {
            for (auto *candidate = item; candidate; candidate = candidate->parentItem())
                if (candidate->property("reorderSourceActive").isValid())
                    return candidate;
            return static_cast<QQuickItem *>(nullptr);
        };
        auto *source    = blockDelegate(sourceHandle);
        auto *following = blockDelegate(followingHandle);
        auto *last      = blockDelegate(lastHandle);
        QVERIFY(source);
        QVERIFY(following);
        QVERIFY(last);
        ...
        for (int step = 1; step <= 6; ++step)
            QTest::mouseMove(&quick, (from + (to - from) * (qreal(step) / 6)).toPoint(), 15);
        ...
        for (int step = 1; step <= 8; ++step) {
            QTest::mouseMove(&quick, (attachFrom + (beforeParagraphSwitch - attachFrom) * (qreal(step) / 8)).toPoint(),
                             15);
            QTest::qWait(50);
        }
