...
ListView {
    ...
    delegate: FocusScope {
        ...
        Loader {
            ...
            sourceComponent: blockType === 1 || blockType === 2 || blockType === 5 ? listEditor
                           ...
                           : blockType === 9 ? tagLineEditor
                           : blockType === 10 ? audioEditor
                           : blockType === 11 ? attachmentEditor : textEditor
        }
    }
    ...
    component BlockTextArea: TextArea {
        ...
        onActiveFocusChanged: {
            if (activeFocus) {
                ...
                root.clearAudioSelection()
                root.clearAttachmentSelection()
    ...
    Component {
        ...
        FocusScope {
            ...
            property var block: parent
            property var playback: root.editorBackend ? root.editorBackend.audioPlayback : null
            property var transcription: root.audioTranscriptionController
            property bool selected: root.selectedAudioIndex === block.index
            ...
            function selectAndFocus() {
                root.selectAudioBlock(block.index)
            ...
            Keys.onPressed: function(event) {
                ...
                if (event.key === Qt.Key_Space || event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                    ...
                    root.removeAudioBlock(block.index, true)
                    event.accepted = true
                } else if (event.key === Qt.Key_Escape) {
                    root.clearAudioSelection()
                    root.forceActiveFocus()
            ...
            ColumnLayout {
                ...
                Rectangle {
                    id: audioCard
                    Layout.fillWidth: true
                    implicitHeight: root.touchMode ? 68 : 58
                    radius: 6
                    ...
                    RowLayout {
                        anchors.fill: parent
                        anchors.margins: root.touchMode ? 8 : 6
                        spacing: 6

                        ToolButton {
                            id: playButton
                            Layout.preferredWidth: root.touchMode ? 44 : 36
                            Layout.preferredHeight: Layout.preferredWidth
                            enabled: audioRoot.playback && audioRoot.playback.available
                            text: audioRoot.loading ? "…" : (audioRoot.playing ? "Ⅱ" : "▶")
                            font.pixelSize: root.touchMode ? 19 : 16
                        ...
                        Label {
                            Layout.minimumWidth: root.touchMode ? 48 : 42
                            ...
                            font.pixelSize: Math.max(10, root.editorFontMetricsHeight * 0.65)
                        }
                        ...
                        ToolButton {
                            ...
                            font.pixelSize: Math.max(9, root.editorFontMetricsHeight * 0.55)
                        ...
                        ToolButton {
                            Layout.preferredWidth: root.touchMode ? 40 : 30
                            Layout.preferredHeight: Layout.preferredWidth
                            ...
                            ToolTip.visible: hovered
                            ToolTip.text: Accessible.name
                            onClicked: root.removeAudioBlock(audioRoot.block.index, true)
                        }
                    }
            ...
            Menu {
                ...
                MenuItem {
                    ...
                    onTriggered: root.removeAudioBlock(audioRoot.block.index, true)
                }
            }
        }
    }
    ...
    Component {
        ...
        FocusScope {
            ...
            property var block: parent
            property bool selected: root.selectedAttachmentIndex === block.index
            width: block.width
            implicitHeight: attachmentCard.implicitHeight
            activeFocusOnTab: true

            function selectAndFocus() {
                root.selectAttachmentBlock(block.index)
            ...
            Keys.onPressed: function(event) {
                if (event.modifiers)
                    return
                if (event.key === Qt.Key_Delete || event.key === Qt.Key_Backspace) {
                    root.removeAttachmentBlock(block.index, true)
                    event.accepted = true
                } else if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                    if (root.platformBackend
                            && typeof root.platformBackend.openAttachment === "function")
                        root.platformBackend.openAttachment(block.url)
                    event.accepted = true
                } else if (event.key === Qt.Key_Escape) {
                    root.clearAttachmentSelection()
                    root.forceActiveFocus()
            ...
            Rectangle {
                id: attachmentCard
                width: parent.width
                implicitHeight: root.touchMode ? 64 : 54
                radius: 6
                ...
                RowLayout {
                    ...
                    Label {
                        Layout.preferredWidth: root.touchMode ? 42 : 34
                        horizontalAlignment: Text.AlignHCenter
                        text: "📎"
                        font.pixelSize: root.touchMode ? 23 : 19
                    }
                    ...
                    ToolButton {
                        ...
                        onClicked: {
                            ...
                            root.platformBackend.openAttachment(attachmentRoot.block.url)
                        }
                    }
                    ...
                    ToolButton {
                        ...
                        ToolTip.visible: hovered
                        ToolTip.text: Accessible.name
                        onClicked: root.removeAttachmentBlock(attachmentRoot.block.index, true)
                    }
                }
            ...
            Menu {
                ...
                MenuItem {
                    ...
                    visible: root.platformBackend
                             && typeof root.platformBackend.openAttachment === "function"
                    onTriggered: root.platformBackend.openAttachment(attachmentRoot.block.url)
                }
                MenuItem {
                    text: qsTr("Save a copy")
                    visible: root.platformBackend
                             && typeof root.platformBackend.saveAttachmentAs === "function"
                    onTriggered: root.platformBackend.saveAttachmentAs(attachmentRoot.block.url)
                }
                MenuSeparator { }
                MenuItem {
                    text: qsTr("Remove Attached File")
                    onTriggered: root.removeAttachmentBlock(attachmentRoot.block.index, true)
                }
            }
        }
    }
