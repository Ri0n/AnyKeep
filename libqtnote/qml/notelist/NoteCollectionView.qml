...
Item {
    ...
    function fallbackIconName(item) {
        ...
        return item.groupKind === "folder" || item.groupKind === "unsorted"
                ? "folder-symbolic" : "qtnote-symbolic"
    }
    ...
    function updateDragCursor() {
        if (!dragging || typeof qtnoteCursor === "undefined" || !qtnoteCursor)
            return
        if (pointerOutsideWindow())
            qtnoteCursor.setTrashCursor()
        else
            qtnoteCursor.setOverrideCursor(Qt.ClosedHandCursor)
    ...
    function restoreDragCursor() {
        if (!dragCursorOverridden)
            return
        if (typeof qtnoteCursor !== "undefined" && qtnoteCursor)
