...
if(NOT ANYKEEP_VERSION_MAJOR)
    ...
    if(Git_FOUND)
      ...
      if(GIT_REPO_VERSION_RESULT EQUAL 0 AND NOT "${GIT_REPO_FULL_VERSION}" STREQUAL "")
        ...
        if(ANYKEEP_VERSION_HASH)
          ...
          # Use the sanitized SemVer core. In particular, strip an optional leading "v" from an exact Git tag before
  ...
  # dpkg-source keeps this file even when the source tree is not a trusted Git checkout. Its first stanza is the package
  # version.
...
# ExternalProject dependencies must use the same active configuration as the parent build. This matters for Visual
# Studio and other multi-config generators, where omitting --config commonly selects Debug even when the AnyKeep target
# is being built as Release.
function(anykeep_external_project_config_args out_var)
  set(_args)
  if(CMAKE_CONFIGURATION_TYPES)
    list(APPEND _args --config "$<CONFIG>")
...
macro(add_anykeep_plugin name description buildable)
  ...
  set(ANYKEEP_COMMON_PLUGIN_SRC ${plugins_SOURCE_DIR}/deintegrationinterface.h
                                ${plugins_SOURCE_DIR}/anykeepplugininterface.h ${plugins_SOURCE_DIR}/trayimpl.h)
  ...
  file(GLOB _plugin_metadata_translations CONFIGURE_DEPENDS "${CMAKE_SOURCE_DIR}/translations/plugin_metadata_*.ts")
  ...
  execute_process(
    COMMAND
      ${Python3_EXECUTABLE} "${CMAKE_SOURCE_DIR}/cmake/generate_plugin_metadata.py" --source
      "${CMAKE_CURRENT_SOURCE_DIR}/${arg_METADATA}" --translations-dir "${CMAKE_SOURCE_DIR}/translations" --icon
      "${CMAKE_CURRENT_SOURCE_DIR}/${arg_ICON}" --output "${_plugin_metadata_output}" --anykeep-version
      "${ANYKEEP_VERSION}"
  ...
  if(NOT _plugin_metadata_result EQUAL 0)
    message(
      FATAL_ERROR "Failed to generate metadata for ${name}:\n${_plugin_metadata_stdout}${_plugin_metadata_stderr}")
  endif()
  ...
  set_property(
    DIRECTORY
    APPEND
    PROPERTY CMAKE_CONFIGURE_DEPENDS "${CMAKE_SOURCE_DIR}/cmake/generate_plugin_metadata.py"
             "${CMAKE_CURRENT_SOURCE_DIR}/${arg_METADATA}" "${CMAKE_CURRENT_SOURCE_DIR}/${arg_ICON}"
...
macro(windeployqt name)
  if(WIN32 AND NOT WINDEPLOYQT_EXECUTABLE)
    ...
    if(TARGET Qt6::qmake)
      get_target_property(_anykeep_qmake_location Qt6::qmake IMPORTED_LOCATION)
  ...
  if(WINDEPLOYQT_EXECUTABLE)
    # Keep CMAKE_INSTALL_PREFIX as an install-time variable.  The update target installs into an isolated staging prefix
    # with `cmake --install --prefix`; baking the configure-time prefix here would make windeployqt copy Qt into the
    # normal install tree instead.
    install(
      CODE "
            execute_process(
...
macro(install_anykeep_plugin name)
  install(
    ...
    RUNTIME DESTINATION ${PLUGINSDIR} COMPONENT Libraries)
  windeployqt(${name})
endmacro()
