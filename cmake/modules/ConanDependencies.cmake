...
function(anykeep_setup_conan_dependencies)
    ...
    execute_process(
        COMMAND "${ANYKEEP_CONAN_COMMAND}" profile path default
    ...
    if(NOT _conan_profile_result EQUAL 0)
        message(FATAL_ERROR
            "Conan's default profile is missing. Run 'conan profile detect' once, "
    ...
    configure_file(
        ...
        "${_conan_output_dir}/conanfile.txt"
        COPYONLY
    )
    set(_conan_common_args
        --profile:host=default
        --profile:build=default
        "--build=${ANYKEEP_CONAN_BUILD_POLICY}"
        "-s:h" "compiler.cppstd=${CMAKE_CXX_STANDARD}"
    )

    if(MSVC)
        string(REGEX MATCH "^19\\.([0-9]+)" _msvc_version_match
            "${CMAKE_CXX_COMPILER_VERSION}")
        if(_msvc_version_match)
            set(_msvc_minor "${CMAKE_MATCH_1}")
            math(EXPR _conan_msvc_version "190 + ${_msvc_minor} / 10")
            list(APPEND _conan_common_args
                "-s:h" "compiler=msvc"
                "-s:h" "compiler.version=${_conan_msvc_version}"
    ...
    elseif(CMAKE_BUILD_TYPE)
        ...
        message(FATAL_ERROR
            "ANYKEEP_USE_CONAN requires CMAKE_BUILD_TYPE for a single-config generator")
    endif()
    ...
    foreach(_build_type IN LISTS _conan_build_types)
        ...
        string(JOIN ";" _conan_signature_input
            "${_conan_version}" "${_conanfile_contents}" "${_conan_args}")
        ...
        if(NOT _installed_signature STREQUAL _conan_signature)
            ...
            execute_process(
                COMMAND "${ANYKEEP_CONAN_COMMAND}" install
                    "${_conan_output_dir}/conanfile.txt"
            ...
            if(NOT _conan_install_result EQUAL 0)
                message(FATAL_ERROR
                    "Conan install failed for ${_build_type} with exit code "
                    "${_conan_install_result}")
            endif()
